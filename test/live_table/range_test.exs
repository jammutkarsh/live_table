defmodule LiveTable.RangeTest do
  ...
  describe "apply/2 for number type" do
    ...
    test "applies range filter for joined field" do
      range = Range.new({:suppliers, :price}, "supplier_price", %{
        type: :number,
        current_min: 20,
        current_max: 200
      })
      ...
      query = from(p in "products", 
        join: s in "suppliers", as: :suppliers, on: p.supplier_id == s.id,
      ...
      assert sql =~ "BETWEEN"
      assert sql =~ "s1"  # suppliers table alias
    end
  ...
  describe "apply/2 for date type" do
    test "applies date range filter for simple field" do
      ...
      range = Range.new(:created_at, "date_range", %{
        type: :date,
      ...
      query = from(p in "products", select: %{id: p.id, created_at: p.created_at}, where: ^dynamic)
      {sql, params} = Ecto.Adapters.SQL.to_sql(:all, Repo, query)
    ...
    test "applies date range filter for joined field" do
      ...
      range = Range.new({:suppliers, :created_at}, "supplier_created", %{
        type: :date,
        current_min: min_date,
        current_max: max_date
      })
  ...
  describe "apply/2 for datetime type" do
    test "applies datetime range filter for simple field" do
      ...
      range = Range.new(:updated_at, "datetime_range", %{
        type: :datetime,
      ...
      query = from(p in "products", select: %{id: p.id, updated_at: p.updated_at}, where: ^dynamic)
      {sql, params} = Ecto.Adapters.SQL.to_sql(:all, Repo, query)
    ...
    test "applies datetime range filter for joined field" do
      ...
      range = Range.new({:suppliers, :updated_at}, "supplier_updated", %{
        type: :datetime,
        current_min: min_datetime,
        current_max: max_datetime
      })
  ...
  describe "render/1" do
    test "renders number range slider with default options" do
      ...
      html = render_component(&Range.render/1, %{
        filter: range,
        key: "price_range",
        applied_filters: %{}
      })
    ...
    test "renders with custom CSS classes" do
      range = Range.new(:value, "value_range", %{
        css_classes: "custom-container",
        slider_classes: "custom-slider",
        label_classes: "custom-label"
      })
